...
- При таймауте не считаем транзакцию неуспешной — проверяем Status API и ждём ответа
- Timeout Reversal отправляем только если PENDING больше 7 дней
- После 30 дней без ответа — автоматически FAILED, резерв возвращается пользователю
